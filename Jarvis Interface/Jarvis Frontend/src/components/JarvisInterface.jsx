...
export default function JarvisInterface() {
  ...
  const animationFrameRef = useRef(null);
  const pulseIntervalRef = useRef(null);
  const audioLevelRef = useRef(0); // Use ref to prevent UI updates from affecting layout
  ...
  const startRecording = async () => {
    try {
      ...
      if (audioContext) {
        ...
        const analyserNode = audioContext.createAnalyser();
        analyserNode.fftSize = 256;
        analyserNode.smoothingTimeConstant = 0.8; // Add smoothing to reduce jitter
        source.connect(analyserNode);
        setAnalyser(analyserNode);
  ...
  const monitorAudioLevel = (analyserNode) => {
    ...
    const updateInterval = 100; // Update every 100ms to reduce jitter
    ...
    const updateLevel = (currentTime) => {
      ...
      if (currentTime - lastUpdateTime >= updateInterval) {
        ...
        const noiseGate = 5; // Minimum threshold
        ...
        if (Math.abs(audioLevelRef.current - audioLevel) > 2) {
